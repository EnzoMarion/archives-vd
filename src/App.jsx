...
// --- Palette de 20 couleurs pour les membres ---
const MEMBER_COLORS = [
...
const CustomTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    return (
        <div className="bg-slate-950/90 border-2 border-blue-500/50 p-4 rounded-xl shadow-[0_0_20px_rgba(59,130,246,0.2)] backdrop-blur-md">
          <p className="text-white font-black uppercase italic tracking-wider mb-1 flex items-center gap-2">
  ...
  return (
      <div className="grid grid-cols-3 gap-4 items-end max-w-3xl mx-auto py-8">
        ...
        <div className="flex flex-col items-center gap-3 animate-in slide-in-from-left-8 duration-700">
          <div className="text-slate-400 font-black italic text-sm uppercase">2nd</div>
          <div className="w-full bg-slate-400/10 border-t-4 border-slate-400 p-6 rounded-t-2xl text-center space-y-2 h-40 flex flex-col justify-center shadow-[0_-10px_30px_rgba(148,163,184,0.1)]">
            <Medal className="w-8 h-8 mx-auto text-slate-400 mb-2" />
            <p className="text-white font-black uppercase italic truncate w-full px-2">{top3[1].name}</p>
            <p className="text-slate-500 font-mono text-xs">{getPoints(top3[1].value)} pts</p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-3 animate-in zoom-in duration-500">
          ...
          <div className="w-full bg-yellow-500/10 border-t-4 border-yellow-500 p-8 rounded-t-3xl text-center space-y-3 h-56 flex flex-col justify-center shadow-[0_-15px_40px_rgba(234,179,8,0.2)]">
            <p className="text-2xl font-black text-white uppercase italic truncate w-full">{top3[0].name}</p>
            <p className="text-yellow-500 font-mono text-sm font-bold">{getPoints(top3[0].value)} pts</p>
            <div className="bg-yellow-500 text-black text-[10px] font-black py-1 px-3 rounded-full uppercase tracking-widest inline-block mx-auto">Champion</div>
          </div>
        </div>

        {/* 3rd Place */}
        <div className="flex flex-col items-center gap-3 animate-in slide-in-from-right-8 duration-700">
          <div className="text-orange-600 font-black italic text-sm uppercase">3rd</div>
          <div className="w-full bg-orange-600/10 border-t-4 border-orange-600 p-6 rounded-t-2xl text-center space-y-2 h-32 flex flex-col justify-center shadow-[0_-10px_30px_rgba(234,88,12,0.1)]">
            <Award className="w-8 h-8 mx-auto text-orange-600 mb-1" />
            <p className="text-white font-black uppercase italic truncate w-full px-2">{top3[2].name}</p>
            <p className="text-slate-500 font-mono text-xs">{getPoints(top3[2].value)} pts</p>
          </div>
        </div>
      </div>
  );
  ...
  const handleImport = () => {
    ...
    try {
      ...
      lines.slice(1).forEach(line => {
        ...
        if (type === 'guild' && nom) {
          guilds.push({ name: nom, points: parseInt(valeur) || 0, color: '#3b82f6' });
        } else if (type === 'member' && nom) {
          members.push({ name: nom, value: parseFloat(valeur) || 0 });
        }
      });
  ...
  if (!isAuthenticated) {
    return (
        <div className="min-h-[60vh] flex items-center justify-center px-4">
          <Card className="p-8 w-full max-w-md space-y-6 text-center shadow-2xl">
            <div className="inline-flex p-4 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-500 mb-2">
              <Shield className="w-8 h-8" />
            </div>
            <h2 className="text-2xl font-black text-white uppercase italic tracking-widest">Zone Restreinte</h2>
            <p className="text-slate-500 text-sm font-bold">Veuillez entrer le code d'accès pour gérer les archives.</p>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-center text-white outline-none focus:border-blue-500 transition-colors font-black tracking-widest"
                  autoFocus
              />
              {error && <p className="text-red-500 text-[10px] font-black uppercase tracking-widest">{error}</p>}
              <button className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black uppercase py-4 rounded-xl transition-all shadow-[0_5px_15px_rgba(37,99,235,0.4)]">
                Accéder
              </button>
              <button type="button" onClick={onBack} className="text-slate-600 text-[10px] font-black uppercase hover:text-white transition-colors">
                Annuler
              </button>
            </form>
          </Card>
  ...
  return (
      <div className="py-12 space-y-12 max-w-4xl mx-auto px-4">
        <div className="flex items-center justify-between">
          <button onClick={onBack} className="flex items-center gap-2 text-slate-500 hover:text-white transition-colors uppercase font-black text-xs tracking-widest">
            <ChevronLeft className="w-4 h-4" /> Retour
          </button>
          <div className="flex items-center gap-2 text-green-500 text-[10px] font-black uppercase tracking-widest bg-green-500/10 px-3 py-1 rounded border border-green-500/20">
            <Unlock className="w-3 h-3" /> Authentifié
          </div>
        </div>

        <div className="space-y-6">
          <h2 className="text-4xl font-black text-white italic uppercase italic tracking-tighter">Gestion des Données</h2>
          ...
          <Card className="p-8 space-y-6 shadow-2xl">
            ...
            <button
                ...
                className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black uppercase py-4 rounded-xl shadow-[0_5px_15px_rgba(37,99,235,0.3)] transition-all flex items-center justify-center gap-2"
            >
          ...
          <div className="space-y-4 pt-8">
            <h3 className="text-xl font-black text-white uppercase italic tracking-tighter">Sessions Enregistrées</h3>
            <div className="grid gap-3">
              {Object.values(sessions).map(s => (
                  <div key={s.id} className="flex items-center justify-between p-4 bg-slate-900/50 border border-slate-800 rounded-xl hover:border-slate-700 transition-colors group">
                    <div>
                      <p className="text-white font-bold">{s.label}</p>
                      <p className="text-slate-500 text-xs uppercase font-black tracking-widest">{s.status}</p>
                    </div>
                    <button
                        onClick={() => onDelete(s.id)}
                        className="p-2 text-slate-600 hover:text-red-500 transition-all bg-red-500/5 rounded-lg border border-transparent hover:border-red-500/20"
                    >
      ...
      <div className="space-y-8 py-8 animate-in fade-in duration-500 px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="p-6 bg-gradient-to-br from-blue-600/10 to-transparent">
            ...
            <p className="text-4xl font-black text-white italic tracking-tighter">{totalLogHorizon.toLocaleString()}</p>
          </Card>
          <Card className="p-6 bg-gradient-to-br from-yellow-500/10 to-transparent">
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">MVP Actuel</p>
            <p className="text-4xl font-black text-white uppercase italic truncate tracking-tighter">{data.members[0]?.name || "N/A"}</p>
          </Card>
          <Card className="p-6 bg-gradient-to-br from-cyan-500/10 to-transparent">
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">Soldats Mobilisés</p>
            <p className="text-4xl font-black text-white italic tracking-tighter">{data.members.length}</p>
          </Card>
          <Card className="p-6 bg-gradient-to-br from-red-500/10 to-transparent">
            ...
            <p className="text-4xl font-black text-white italic tracking-tighter">TOP 1</p>
          </Card>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <Card className="p-8">
            <h3 className="text-xl font-black text-white uppercase italic mb-8 flex items-center gap-2 tracking-tighter">
              <BarChart3 className="w-5 h-5 text-blue-500" /> Puissance des Guildes
            </h3>
          ...
          <Card className="p-8">
            <h3 className="text-xl font-black text-white uppercase italic mb-8 flex items-center gap-2 tracking-tighter">
              <Zap className="w-5 h-5 text-yellow-500" /> Répartition Membres
            </h3>
            <div className="h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  >
                    {data.members.map((_, index) => (
                        <Cell
                            ...
                            stroke="#020203"
                            strokeWidth={2}
                            className="hover:scale-105 transition-transform duration-300 origin-center cursor-pointer"
                        />
                    ))}
                  </Pie>
        ...
        <Card className="shadow-2xl">
          <div className="p-6 border-b border-slate-800 flex items-center gap-2">
            ...
            <h3 className="text-xl font-black text-white uppercase italic tracking-tighter">Leaderboard Individuel</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
              <tr className="bg-slate-950/50 text-slate-500 text-[10px] font-black uppercase tracking-widest font-mono">
                <th className="px-8 py-5">Rang</th>
                <th className="px-8 py-5">Membre</th>
                <th className="px-8 py-5 text-right">Contribution</th>
                <th className="px-8 py-5 text-right font-mono tracking-tighter">Points Estimés</th>
              </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50 text-sm">
              {data.members.map((m, i) => (
                  <tr key={m.name} className="hover:bg-blue-600/5 transition-colors group">
                    <td className="px-8 py-4"><span className={`w-8 h-8 rounded flex items-center justify-center font-black ${i < 3 ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400'}`}>{i+1}</span></td>
                    <td className="px-8 py-4 text-white font-black uppercase italic tracking-wider group-hover:text-blue-400 transition-colors">{m.name}</td>
                    <td className="px-8 py-4 text-right text-cyan-400 font-mono font-black">{m.value}%</td>
                    <td className="px-8 py-4 text-right text-slate-500 font-mono italic tracking-tighter">
                      {Math.round((data.totalPointsLog * m.value) / 100).toLocaleString()}
                      <span className="text-[10px] opacity-40 font-black ml-1 uppercase">Pts</span>
                    </td>
                  </tr>
              ))}
              </tbody>
...
export default function App() {
  ...
  const handleDelete = (id) => {
    const confirm = window.confirm("Supprimer cette session ?");
    if (confirm) {
  ...
  return (
      <div className="min-h-screen bg-[#020203] text-slate-200 font-sans selection:bg-blue-600 selection:text-white pb-10">
        ...
        <main className="max-w-7xl mx-auto">
          {view === 'home' && (
              <HomePage
                  sessions={sessions}
                  onSelectSession={(id) => { setSelectedId(id); setView('stats'); }}
